...
def support_multi_agent_workflow(ctx: wf.DaprWorkflowContext, input_data: dict):
...
def start_workflow(request: SupportRequest):
    try:
        ...
        logger.info(f"Starting support multi-agent workflow for {request.customer}")
        workflow_client.schedule_new_workflow(
            workflow=support_multi_agent_workflow,
            input=request.dict(),
